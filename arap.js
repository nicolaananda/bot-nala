require('./settings')
const { BufferJSON, WA_DEFAULT_EPHEMERAL, generateWAMessageFromContent, proto, generateWAMessageContent, generateWAMessage, prepareWAMessageMedia, areJidsSameUser, getContentType, downloadContentFromMessage } = require("@dappaoffc/baileys")
const fs = require('fs');
...
// Helper: Generate attendance invoice with 4 photos
async function generateAttendanceInvoice(attendances) {
    ...
    // Format as RRGGBB (hex) then convert to RGB values
    const textColor = 0x70370D; // RGB: 112, 55, 13
    
    // Load default Jimp fonts
    // fontSmall: untuk Date, For (nama murid) - ukuran tetap
    // fontCaption: untuk caption gambar (tanggal, deskripsi, harga) - ukuran lebih besar
    // fontMedium: untuk Total Payment
    let fontSmall, fontMedium, fontCaption;
    try {
        fontSmall = await Jimp.loadFont(Jimp.FONT_SANS_32_BLACK);  // Untuk Date, For - ukuran tetap
        fontMedium = await Jimp.loadFont(Jimp.FONT_SANS_64_BLACK); // Untuk Total Payment
        fontCaption = await Jimp.loadFont(Jimp.FONT_SANS_64_BLACK); // Untuk caption gambar - ukuran besar
    } catch (err) {
        // Fallback to smaller sizes if large fonts not available
        try {
    ...
    // Color format: 0xRRGGBB where #70370d = rgb(112, 55, 13)
    const printTextWithColor = (img, font, x, y, text, color = textColor) => {
        // Extract RGB from color (0x70370D = RRGGBB format)
        // For 0x70370D: RR=70(hex)=112, GG=37(hex)=55, BB=0D(hex)=13
        const r = (color >> 16) & 0xFF; // Extract RR (bits 16-23) = 0x70 = 112
        const g = (color >> 8) & 0xFF;  // Extract GG (bits 8-15) = 0x37 = 55
        const b = color & 0xFF;          // Extract BB (bits 0-7) = 0x0D = 13
        
        // Create temporary image for text rendering
        const textWidth = Jimp.measureText(font, text);
        const textHeight = Jimp.measureTextHeight(font, text);
        const padding = 10;
        const tempImg = new Jimp(textWidth + padding * 2, textHeight + padding * 2, 0x00000000); // Transparent
        
        // Print text on temp image (black)
        tempImg.print(font, padding, padding, text);
        
        // Colorize text pixels (replace black with custom color)
        tempImg.scan(0, 0, tempImg.bitmap.width, tempImg.bitmap.height, function (x, y, idx) {
            ...
            // If pixel is text (dark/black pixels with alpha > 0), replace with custom color
        ...
        // Composite colored text onto main image (main print)
        img.composite(tempImg, x - padding, y - padding);
        
        // Bold effect: print again with 1px offset in both directions for thicker appearance
        const boldOffset = 1;
    ...
    // Get current date for invoice (date when invoice is created)
    const invoiceDate = moment().format('DD/MM/YYYY');
    
    // Calculate total price
    const totalHarga = attendances.reduce((sum, att) => sum + att.harga, 0);
    
    // Fill in invoice fields in template with corrected precise coordinates
    // Date field - after "Date :" label
    printTextWithColor(invoice, fontSmall, 260, 468, invoiceDate);
    
    // For field - after "For :" label  
    printTextWithColor(invoice, fontSmall, 310, 515, namaMurid);
    

    // Photo dimensions based on template
    const photoWidth = 320;
    const photoHeight = 320;
    
    // Photo positions for 2x2 grid (corrected precise coordinates - all moved down by 40px)
    ...
    // Determine how many photos to display (max 4)
    const photoCount = Math.min(attendances.length, 4);
    
    // Process photos using exact template coordinates
    for (let i = 0; i < photoCount; i++) {
        ...
        try {
            ...
            // Print date, description, and price below photo with custom color (gunakan fontCaption yang lebih besar)
...
function Styles(text, style = 1) {
    ...
    // Handle photo with caption for attendance (semua user bisa menggunakan)
    ...
    if (m.mtype === 'imageMessage' && body && body.trim()) {
        try {
            ...
            // Check if caption starts with "absen" (case insensitive)
            ...
            // Remove "absen" from the start and parse the rest
            // Handle both "absen" and "absen " (with or without space)
            const captionWithoutTrigger = body.trim().replace(/^absen\s+/i, '').trim();
            if (!captionWithoutTrigger) {
                return await reply('⚠️ Format caption salah! Setelah *absen* harus diikuti data.\n\nFormat: *absen* NamaMurid Harga Tanggal(DD/MM/YYYY) DeskripsiKelas\nContoh: *absen* Andi 100000 02/11/2025 Kelas Gitar Dasar');
            }
            ...
            if (captionParts.length >= 4) {
                ...
                if (isNaN(harga) || harga <= 0) {
                    return await reply('⚠️ Format caption salah! Harga harus berupa angka.\n\nFormat: *absen* NamaMurid Harga Tanggal(DD/MM/YYYY) DeskripsiKelas\nContoh: *absen* Andi 100000 02/11/2025 Kelas Gitar Dasar');
                }
                
                // Validate and parse tanggal (DD/MM/YYYY)
                const tanggalMatch = tanggalStr.match(/^(\d{2})\/(\d{2})\/(\d{4})$/);
                if (!tanggalMatch) {
                    return await reply('⚠️ Format tanggal salah! Gunakan format DD/MM/YYYY.\n\nFormat: *absen* NamaMurid Harga Tanggal(DD/MM/YYYY) DeskripsiKelas\nContoh: *absen* Andi 100000 02/11/2025 Kelas Gitar Dasar');
                }
                
                const [, day, month, year] = tanggalMatch;
                const tanggal = moment(`${year}-${month}-${day}`, 'YYYY-MM-DD');
                if (!tanggal.isValid()) {
                    return await reply('⚠️ Tanggal tidak valid!');
                }
                
                // Check if message has image and get image message object
                const imageMsg = m.msg || m.message?.imageMessage;
                if (!imageMsg) {
                    return await reply('⚠️ Pesan tidak mengandung foto yang valid. Pastikan foto sudah terkirim dengan lengkap.');
                }
                
                // Check if media key exists
                if (!imageMsg.mediaKey || !imageMsg.url) {
                    // Wait longer and try to get fresh message
                    await sleep(2000);
                    
                    // Check again after delay
                    const freshImageMsg = m.msg || m.message?.imageMessage;
                    if (!freshImageMsg?.mediaKey || !freshImageMsg?.url) {
                        return await reply('⚠️ Foto belum siap untuk didownload.\n\nSilakan tunggu beberapa detik dan kirim ulang foto dengan caption yang sama.\n\nPastikan foto sudah benar-benar terkirim sebelum mencoba lagi.');
                    }
                }
                
                // Download and save photo with retry mechanism using downloadContentFromMessage directly
                let photoBuffer;
                let retryCount = 0;
                ...
                while (retryCount < maxRetries) {
                    try {
                        // Add delay on retries (longer delays)
                        if (retryCount > 0) {
                            await sleep(2000 * retryCount); // Wait 2s, 4s, 6s, 8s, 10s on retries
                        }
                        
                        // Get fresh message reference on retry
                        ...
                        // Use downloadContentFromMessage directly like downloadAndSaveMediaMessage does
                        const mime = currentImageMsg.mimetype || 'image/jpeg';
                        const messageType = mime.split('/')[0]; // 'image'
                        ...
                        retryCount++;
                        console.error(`Error downloading photo (attempt ${retryCount}/${maxRetries}):`, err.message);
                        
                        // Check if error is about empty media key
                        if (err.message && (err.message.includes('empty media key') || err.message.includes('media key') || err.message.includes('not available'))) {
                            if (retryCount >= maxRetries) {
                                return await reply(`⚠️ Gagal mendownload foto setelah ${maxRetries} percobaan.\n\nKemungkinan media belum siap atau foto belum sepenuhnya terkirim.\n\n**Solusi:**\n1. Tunggu 5-10 detik setelah mengirim foto\n2. Kirim ulang foto dengan caption yang sama\n3. Pastikan koneksi internet stabil\n\nError: ${err.message}`);
                            }
                        } else if (retryCount >= maxRetries) {
                            return await reply(`⚠️ Gagal mendownload foto setelah ${maxRetries} percobaan.\n\nPastikan foto sudah terkirim dengan lengkap dan coba lagi.\n\nError: ${err.message}`);
                        }
                    }
                }
                ...
                const uninvoicedAttendances = await Attendance.find({ 
                    nama: nama,
                    isInvoiced: false // Only get attendances that haven't been invoiced
                })
...
case 'invoice': {
    try {
        ...
        // Find all attendances for this student that haven't been invoiced yet (case-insensitive search)
        ...
        if (allStudentAttendances.length === 0) {
            return await reply(`⚠️ Tidak ditemukan data absen yang belum masuk invoice untuk *${nama}*.\n\nSemua absen sudah masuk invoice sebelumnya.`);
        }
        ...
        // Generate invoice (support any number of attendances >= 1)
        try {
            ...
            console.error('Error generating invoice:', invoiceErr);
            return await reply(`⚠️ Gagal membuat invoice untuk *${actualNama}*: ${invoiceErr.message}`);
        }
    } catch (err) {
